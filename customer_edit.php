...
if ($id <= 0) {
  header('Location: customers.php?error=' . urlencode('ไม่พบลูกค้า'));
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    ...
    header('Location: customers.php?ok=' . urlencode('แก้ไขลูกค้าเรียบร้อย'));
    exit;
  }
}
?>
        ...
        <form method="post" id="editForm" novalidate>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:10px;">
            ...
            <div>
              ...
              <input class="input" type="tel" name="phone" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}"value="<?= htmlspecialchars($_POST['phone'] ?? $row['phone']) ?>" required>
            </div>
          </div>
