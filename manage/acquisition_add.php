...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  // ถ้าต้องการกันซื้อซ้ำคันเดิม เปิดคอมเมนต์ด้านล่าง
  // $dup = $pdo->prepare("SELECT 1 FROM acquisitions WHERE machine_id=? LIMIT 1");
  // $dup->execute([$machine_id]); if ($dup->fetch()) $errors[]='มีเอกสารซื้อสำหรับคันนี้แล้ว';

  if (!$errors) {
    $vat_amount  = round($base_price * ($vat_rate/100), 2);
    $total       = round($base_price + $vat_amount, 2);

    $ins = $pdo->prepare("INSERT INTO acquisitions
      (machine_id,supplier_id,doc_no,acquired_at,base_price,vat_rate_pct,vat_amount,total_amount,remark)
      VALUES (?,?,?,?,?,?,?,?,?)");
    $ins->execute([$machine_id,$supplier_id,$doc_no,$acquired_at,$base_price,$vat_rate,$vat_amount,$total,$remark]);

    header('Location: acquisitions.php?ok=' . urlencode('เพิ่มเอกสารซื้อเรียบร้อย')); exit;
  }
}
?>
<!doctype html>
...
<title>เพิ่มเอกสารซื้อ — ProInspect Machinery</title>
...
<div class="layout">
  ...
  <main class="content">
    <div class="page-head">
      <h2 class="page-title">เพิ่มเอกสารซื้อ</h2>
      <div class="page-sub">บันทึกการซื้อรถเข้า</div>
    </div>
    ...
    <section class="card">
      <form method="post" id="addForm" novalidate>
        ...
        <div class="row" style="margin-top:10px;">
          ...
          <div><label>วันที่ซื้อ</label><input class="input" type="date" name="acquired_at" required value="<?=htmlspecialchars($_POST['acquired_at'] ?? date('Y-m-d'))?>"></div>
        </div>

        <div class="row" style="margin-top:10px;">
          <div><label>ราคาก่อน VAT</label><input class="input" type="number" step="0.01" name="base_price" required value="<?=htmlspecialchars($_POST['base_price'] ?? '')?>"></div>
...
document.getElementById('addForm').addEventListener('submit',(e)=>{
  ...
  const bp  = parseFloat(document.querySelector('[name="base_price"]').value||'NaN');
  ...
  if(isNaN(bp)){ Swal.fire({icon:'warning',title:'กรอกราคาก่อน VAT',confirmButtonColor:'#fec201'}); return; }
