...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (!$errors) {
    $ins = $pdo->prepare("INSERT INTO machine_expenses
      (machine_id,supplier_id,category,capitalizable,description,occurred_at,qty,unit_cost,commission_amt,status,remark)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $ins->execute([$machine_id,$supplier_id,$category,$capitalizable,$description,$occurred_at,$qty,$unit_cost,$commission,$status,$remark]);
    header('Location: expenses.php?ok=' . urlencode('เพิ่มค่าใช้จ่ายเรียบร้อย')); exit;
  }
}
?>
<!doctype html>
  ...
  <main class="content">
    ...
    <section class="card">
      <form method="post" id="addForm" novalidate>
          ...
          <div>
            ...
            <select class="select" name="category" required>
              <?php foreach($catMap as $k=>$v): ?>
                <option value="<?=$k?>" <?= (($_POST['category'] ?? '')===$k)?'selected':''; ?>><?=$v?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="row" style="margin-top:10px;">
          <div>
            ...
            <select class="select" name="capitalizable">
              <option value="1" <?= (($_POST['capitalizable'] ?? '1')==='1')?'selected':''; ?>>เข้าทุน</option>
              <option value="0" <?= (($_POST['capitalizable'] ?? '')==='0')?'selected':''; ?>>ค่าใช้จ่ายงวด</option>
            </select>
          </div>
          <div>
        ...
        <div class="row" style="margin-top:10px;">
          <div><label>ค่านายหน้า (ถ้ามี)</label><input class="input" type="number" step="0.01" name="commission_amt" value="<?=htmlspecialchars($_POST['commission_amt'] ?? '0.00')?>" oninput="calcPreview()"></div>
          <div>
            <label>สถานะ</label>
            <select class="select" name="status">
              <?php foreach([1,2,3,4,9] as $opt): ?>
                <option value="<?=$opt?>" <?= (int)($_POST['status'] ?? 1)===$opt?'selected':''; ?>><?=exp_status_label($opt)?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
