...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  header('Location: sales.php?error=' . urlencode('วิธีการเรียกไม่ถูกต้อง')); exit;
}
...
function back_path(): string {
  ...
  return rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\') . '/sales.php';
}
...
$del = $pdo->prepare("DELETE FROM sales WHERE sale_id=? LIMIT 1");
$del->execute([$id]);

if ($del->rowCount()>0) redirect_with(back_path(), ['ok'=>'ลบเอกสารขายเรียบร้อย']);
