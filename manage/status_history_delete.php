...
if ($_SERVER['REQUEST_METHOD']!=='POST') {
  header('Location: status_history.php?error=' . urlencode('วิธีการเรียกไม่ถูกต้อง')); exit;
}
if (empty($_POST['csrf']) || !hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'])) {
  header('Location: status_history.php?error=' . urlencode('CSRF token ไม่ถูกต้อง')); exit;
}
function back_path(): string {
  ...
  return rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\') . '/status_history.php';
}
...
$del = $pdo->prepare("DELETE FROM machine_status_history WHERE history_id=? LIMIT 1");
$del->execute([$id]);
if ($del->rowCount()>0) redirect_with(back_path(), ['ok'=>'ลบประวัติเรียบร้อย']);
