...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    $ins = $pdo->prepare('INSERT INTO suppliers (supplier_name,tax_id,phone,email,address) VALUES (?,?,?,?,?)');
    $ins->execute([$name,$tax,$phone,$email,$addr]);
    header('Location: suppliers.php?ok=' . urlencode('เพิ่มผู้ขายเรียบร้อย')); exit;
  }
}
?>
<!doctype html>
  ...
  <main class="content">
    ...
    <section class="card">
      <form method="post" id="supForm" novalidate>
        ...
        <input class="input" type="text" name="supplier_name" required value="<?=htmlspecialchars($_POST['supplier_name'] ?? '')?>">
...
document.getElementById('supForm').addEventListener('submit', (e) => {
  ...
  if(!name){
    Swal.fire({icon:'warning', title:'กรอกชื่อผู้ขาย', confirmButtonColor:'#fec201'}); return;
  }
  Swal.fire({
    icon:'question', title:'ยืนยันการบันทึก?', text:`ผู้ขาย: "${name}"`,
